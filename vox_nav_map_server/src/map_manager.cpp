  ...
  map_configured_(false)
{
  ...
  RCLCPP_INFO(
    this->get_logger(), "Loaded a PCD map with %ld points",
...
void MapManager::preProcessPCDMap()
{
  ...
  pcd_map_pointcloud_ =
    vox_nav_map_server::set_cloud_color(
      pcd_map_pointcloud_,
...
void MapManager::regressCosts()
{
  ...
  auto pure_non_traversable_pcl = vox_nav_map_server::get_non_traversable_points(
    pcd_map_pointcloud_);
  ...
  auto surfels = vox_nav_map_server::surfelize_traversability_cloud(
    pure_traversable_pcl,
  ...
  for (auto && i : surfels) {
    ...
    try {
      vox_nav_map_server::fit_plane_to_cloud(
        plane_model,
        surfel_cloud,
      ...
      RCLCPP_ERROR(
        get_logger(),
        "Current surfel has %ld points, Jumping to next surfel", surfel_cloud->points.size());
      continue;
    }
    ...
    double average_point_deviation = vox_nav_map_server::average_point_deviation_from_plane(
      surfel_cloud,
      *plane_model);
