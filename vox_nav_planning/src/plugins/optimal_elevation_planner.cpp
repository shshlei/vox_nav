...
void OptimalElevationPlanner::initialize(rclcpp::Node * parent)
{
  is_map_ready_ = false;

  elevated_surfel_cloud_ = pcl::PointCloud<pcl::PointSurfel>::Ptr(new pcl::PointCloud<pcl::PointSurfel>);
  elevated_traversable_cloud_ = pcl::PointCloud<pcl::PointXYZRGB>::Ptr(new pcl::PointCloud<pcl::PointXYZRGB>);
  ...
  supervoxel_disable_transform_ = parent->declare_parameter("supervoxel_disable_transform", false);
  supervoxel_resolution_ = parent->declare_parameter("supervoxel_resolution", 0.8);
  supervoxel_seed_resolution_ = parent->declare_parameter("supervoxel_seed_resolution", 1.0);
  supervoxel_color_importance_ = parent->declare_parameter("supervoxel_color_importance", 0.0);
  supervoxel_spatial_importance_ = parent->declare_parameter("supervoxel_spatial_importance", 1.0);
  supervoxel_normal_importance_ = parent->declare_parameter("supervoxel_normal_importance", 1.0);
  distance_penalty_weight_ = parent->declare_parameter("distance_penalty_weight", 1.0);
  elevation_penalty_weight_ = parent->declare_parameter("elevation_penalty_weight", 1.0);
  graph_search_method_ = parent->declare_parameter("graph_search_method", "astar");
...
std::vector<geometry_msgs::msg::PoseStamped> OptimalElevationPlanner::createPlan(const geometry_msgs::msg::PoseStamped & start, const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  auto search_area_surfels = vox_nav_utilities::getSubCloudWithinRadius<pcl::PointXYZRGB>(elevated_traversable_cloud_, search_point_xyzrgb, radius);

  auto search_area_rgba_pointcloud = pcl::PointCloud<pcl::PointXYZRGBA>::Ptr(new pcl::PointCloud<pcl::PointXYZRGBA>);
...
bool OptimalElevationPlanner::isStateValid(const ompl::base::State * state)
{
  ...
  // float distance = vox_nav_utilities::distance(xyzv->values[0], xyzv->values[1], xyzv->values[2], elevated_point_cloud_);
  // bool valid = (distance <= 0.5);

  // RCLCPP_INFO(logger_, "State %.4f, %.4f, %.4f, %.4f", xyzv->values[0], xyzv->values[1], xyzv->values[2], so2->value);
  // RCLCPP_INFO(logger_, "First collision %d, second collision %d", valid, collisionWithFullMapResult.isCollision());
  // return valid && !collisionWithFullMapResult.isCollision();
  return !collisionWithFullMapResult.isCollision();
}

bool OptimalElevationPlanner::isEdgeinCollision(const pcl::PointXYZRGBA & a, const pcl::PointXYZRGBA & b)
{
