...
void OSMElevationPlanner::initialize(rclcpp::Node * parent)
{
  is_map_ready_ = false;
  ...
  goal_tolerance_ = parent->declare_parameter("goal_tolerance", 2.0);
  double minv = parent->declare_parameter("control_boundries.minv", -0.5);
  double minw = parent->declare_parameter("control_boundries.minw", -0.5);
  double maxv = parent->declare_parameter("control_boundries.maxv", 0.5);
...
void OSMElevationPlanner::setupMap()
{
  ...
  while (!is_map_ready_ && rclcpp::ok()) {
    ...
    while (!get_osm_road_topology_map_client_->wait_for_service(
      ...
      RCLCPP_INFO(
        logger_,
        "get_osm_map service not available, "
