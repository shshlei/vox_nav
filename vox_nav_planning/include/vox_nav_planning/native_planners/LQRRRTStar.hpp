...
protected:
  ...
  Node * steer(Node * from_node, Node * to_node, double * relative_cost)
  {
    ...
    if (resulting_path.size() > 1) {
      for (std::size_t i = 1; i < resulting_path.size(); i++) {
  ...
  double relative_path_cost(Node * from_node, Node * to_node)
  {
    ...
    if (to_node->path_.size() > 2) {
      for (std::size_t i = 1; i < to_node->path_.size(); i++) {
  ...
  void smooth_final_course(Node * last_valid_node, std::size_t segment_framing, Node * /*goal_node*/)
  {
    lqr_planner_->set_max_time(50.0);
    ...
    for (std::size_t i = segment_framing; i < path_nodes.size(); i += segment_framing) {
      ...
      if (new_node) {
        ...
        if (no_collision && improved_cost) {
          ...
          for (std::size_t j = i - segment_framing; j < path_nodes.size(); j++) {
